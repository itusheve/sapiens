...
let AuthController = class AuthController {
    ...
    async login(loginDto, response) {
        try {
            ...
            if (!user) {
                return response.status(common_1.HttpStatus.UNAUTHORIZED).json({ message: 'Invalid credentials' });
            }
            ...
            return response.status(common_1.HttpStatus.OK).json({ accessToken });
        }
        catch (error) {
    ...
    async getUserByCrededentials(username, password) {
        try {
